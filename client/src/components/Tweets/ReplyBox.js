...
const ReplyBox = ({tweet}) => {
  ...
  return (
    <>
        { !hiddenDiv && <div className='replyingTo'>Replying to <span className='replyingToUsername'>@{tweet.username}</span></div>}
        ...
        <div className={!hiddenDiv ? 'borderBot submitReplyDiv' : 'testing borderBot'}>
            <button className='submitReplyButton' onClick={createReply}>Reply</button>
        </div>
    </>
  )
}
